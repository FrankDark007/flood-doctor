...
/**
 * LandingPageB - SaaS Dashboard Style Template
 *
...
interface LandingPageBProps {
  ...
  dashboardStatus?: { title: string; subtitle: string; percent: number };
  dashboardMetrics?: DashboardMetric[];
  dashboardChartData?: number[];
...
const LandingPageB: React.FC<LandingPageBProps> = ({
  pageTitle = 'Emergency Water Damage Restoration & Flood Cleanup',
  pageDescription = '24/7 emergency water damage restoration in Northern Virginia. Real-time monitoring, direct insurance billing, 60-minute response.',
  heroEyebrow = 'Crews Available Now',
  heroEyebrowIcon = true,
  heroTitle = <>Water damage restoration, <span className="text-primary">simplified.</span></>,
  heroSubtitle = 'Track your restoration in real-time. From emergency dispatch to final walkthrough—complete visibility at every step.',
  heroCTAPrimary = { label: '(877) 497-0007', href: 'tel:8774970007' },
  heroCTASecondary = { label: 'Get Free Estimate', to: '/request/' },
  showDashboard = true,
  dashboardTitle = 'Flood Doctor Client Portal',
  dashboardStatus = { title: 'Drying in Progress', subtitle: 'Day 3 of 5 • 62% Complete', percent: 62 },
  dashboardMetrics = [
    { label: 'Moisture Level', value: '18%', sublabel: '↓ 4% from yesterday', icon: Thermometer, trend: 'down' },
    { label: 'Equipment', value: '8', sublabel: 'Units active', icon: Wifi },
    { label: 'Est. Completion', value: 'Jan 5', sublabel: '2 days left', icon: Clock }
  ],
  dashboardChartData = [85, 72, 58, 45, 38, 28, 18],
  stats = [
  ...
  processTitle = 'From call to restoration',
  processSubtitle = 'Our streamlined response process',
  processSteps = [
  ...
  servicesTitle = 'Restoration services',
  servicesSubtitle = 'Complete solutions for homes and businesses.',
  serviceTabs = [
    {
      id: 'residential',
      label: 'Residential',
  ...
  featuresEyebrow = 'Technology',
  featuresTitle = 'Stay informed at every step',
  featuresSubtitle = 'Our client portal gives you complete visibility into your restoration project.',
  features = [
    {
      icon: Gauge,
  ...
  trustBadges = [
    { icon: Shield, title: 'IICRC Certified', subtitle: 'Industry standard' },
    { icon: Award, title: 'BBB A+ Rated', subtitle: '15+ years' },
    { icon: Star, title: '4.9/5 Rating', subtitle: '500+ reviews' },
    { icon: Clock, title: '60 Min Response', subtitle: 'Guaranteed' }
  ],
  locationsEyebrow = 'Coverage Area',
  locationsTitle = 'Northern Virginia & DC Metro',
  locationsSubtitle = 'Strategic fleet positioning ensures we reach any location in our service area within 60 minutes—day or night.',
  locations = ['Fairfax County', 'Arlington', 'Alexandria', 'Loudoun County', 'Prince William', 'Washington DC'],
  ctaTitle = 'Get started in minutes',
  ctaSubtitle = 'Call now for immediate dispatch or schedule a free estimate online.'
}) => {
  const [activeTab, setActiveTab] = useState(serviceTabs[0]?.id || 'residential');
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-700 text-sm font-medium mb-6">
                {heroEyebrowIcon && <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />}
                {heroEyebrow}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-10">
                <Button href={heroCTAPrimary.href} variant="primary" size="lg" className="h-14 px-8">
                  <Phone size={20} className="mr-2" />
                  {heroCTAPrimary.label}
                </Button>
                <Button to={heroCTASecondary.to} variant="outline" size="lg" className="h-14 px-8">
              ...
              <div className="relative">
                <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
                  ...
                  <div className="bg-slate-800 px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="text-white/60 text-sm">{dashboardTitle}</div>
                    <div className="w-20" />
                  </div>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-grow">
                        <div className="font-semibold text-emerald-800">{dashboardStatus.title}</div>
                        <div className="text-sm text-emerald-600">{dashboardStatus.subtitle}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-display font-bold text-emerald-700">{dashboardStatus.percent}%</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {dashboardMetrics.map((metric, idx) => (
                        <div key={idx} className="bg-slate-50 rounded-xl p-4">
                          ...
                          <div className="font-display text-lg font-bold text-text">{metric.value}</div>
                          {metric.sublabel && (
                            <div className={`text-xs ${metric.trend === 'down' ? 'text-emerald-600' : 'text-muted'}`}>
                              {metric.sublabel}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="bg-slate-50 rounded-xl p-4">
                      ...
                      <div className="h-20 flex items-end gap-2">
                        {dashboardChartData.map((val, idx) => (
                          <div
                            key={idx}
              ...
              {trustBadges.map((badge, idx) => (
                <div key={idx} className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center">
                    <badge.icon className={badge.title.includes('Rating') ? 'text-amber-500' : 'text-primary'} size={28} />
                  </div>
                  <div>
                    <div className="font-semibold text-text">{badge.title}</div>
