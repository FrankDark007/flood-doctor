...
## CityLift P5 — Canonical & Sitemap Structural Cleanup (2026-02-19)

**Goal:** Align all city service page canonical URLs to nested served paths and eliminate duplicate sitemaps from main domain.

**Phase 1 — Canonical Audit:**
- Audited all 13 cities × 8 services (104 files + 1 Arlington emergency-water-removal)
- Found 87 files with flat-path canonicals (e.g., `/mold-remediation` instead of `/services/residential/cleanup-services/mold-remediation/`)
- 8 files already correct from P4.2 (burst-pipe + fire-damage × 4 P3b cities)
- 9 files (5 Alexandria, 4 Tysons) use older `ServicePageContent` format without `meta.canonical` field — no action needed

**Phase 2 — Canonical Fix:**
- Fixed 87 files via mechanical sed replacement
- All 8 service types mapped to correct nested paths per `data/city-service-map.ts`
- Handled both double-quoted and single-quoted (Vienna) canonical values
- Build verified: 189/189 ✅

**Phase 3 — Sitemap Cleanup:**
- Discovered two competing sitemap generators: `generate-sitemaps.ts` (legacy, flat URLs) and `build-cities.ts:generateCitySitemap()` (correct, nested URLs)
- Stripped city sitemap generation from `generate-sitemaps.ts` (kept main-site-only)
- Deleted 13 stale city sitemap XMLs from `public/sitemaps/`
- City sitemaps now solely generated by `build-cities.ts` with correct nested paths

**Key Discovery:**
`meta.canonical` in city service content files is NOT rendered in HTML output. `PageMeta` component auto-generates canonical from the browser URL path. Fixes done for data consistency.

**Commits:**
- `a80ac11` — CityLift P5 — align all city service canonicals to nested served paths (87 files)
- `8d702ed` — CityLift P5 — remove legacy city sitemaps from main site generator
